...
The data directory is the default location where the Zetacoin Core files are stored.
...
---------|--------------------
Linux    | `$HOME/.zetacoin/`
macOS    | `$HOME/Library/Application Support/Zetacoin/`
Windows  | `%LOCALAPPDATA%\Zetacoin\` <sup>[\[1\]](#note1)</sup>

2. A custom data directory path can be specified with the `-datadir` option.

3. All content of the data directory, except for `zetacoin.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Zetacoin blocks (dumped in network format, 128 MiB per file)
...
`./`               | `zetacoin.conf`       | User-defined [configuration settings](zetacoin-conf.md) for `zetacoind` or `zetacoin-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
`./`               | `zetacoind.pid`       | Stores the process ID (PID) of `zetacoind` or `zetacoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `zetacoind` or `zetacoin-qt`; can be specified by `-debuglogfile` option
...
`./`               | `settings.json`       | Read-write settings set through GUI or RPC interfaces, augmenting manual settings from [zetacoin.conf](zetacoin-conf.md). File is created automatically if read-write settings storage is not disabled with `-nosettings` option. Path can be specified with `-settings` option
...
`zetacoin-qt` uses [`QSettings`](https://doc.qt.io/qt-6/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-6/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by Zetacoin Core:
...
This table describes the files installed by Zetacoin Core across different platforms.

| **Path**                                                   | **Description**                                                             |
|------------------------------------------------------------|-----------------------------------------------------------------------------|
| [README.md](README.md) or [readme.txt](README_windows.txt) | Project information and instructions                                        |
| zetacoin.conf                                              | Configuration file                                                          |
| bin/zetacoin                                               | Command-line tool for interacting with Zetacoin. Calls other binaries below.|
| bin/zetacoin-cli                                           | Tool for making node and wallet RPC calls.                                  |
| bin/zetacoin-qt                                            | Zetacoin node and wallet GUI                                                |
| bin/zetacoin-tx                                            | Tool for creating and modifying transactions                                |
| bin/zetacoin-util                                          | Miscellaneous utilities                                                     |
| bin/zetacoin-wallet                                        | Zetacoin wallet tool                                                        |
| bin/zetacoind                                              | Zetacoin node and wallet daemon                                             |
...
| *libexec/bench_zetacoin*                                   | Benchmarking tool for measuring node performance                            |
| *libexec/zetacoin-chainstate*                              | Tool to validate and connect blocks                                         |
| libexec/zetacoin-gui                                       | IPC-enabled alternative to `zetacoin-qt`                                    |
| libexec/zetacoin-node                                      | IPC-enabled alternative to `zetacoind`                                      |
| libexec/test_zetacoin                                      | Unit test binary                                                            |
| *libexec/test_zetacoin-qt*                                 | GUI-specific unit tests                                                     |
| share/man/man1/                                            | Man pages for command-line tools like `zetacoin-cli`, `zetacoind`, and others|
...
- README and zetacoin.conf files are included in binary releases but not installed in source builds.
- On Windows, binaries have a `.exe` suffix (e.g., `zetacoin-cli.exe`).
...
There have been multiple reports of database corruption and data loss when using this filesystem with Bitcoin Core (upstream), see [Issue #31454](https://github.com/bitcoin/bitcoin/issues/31454) for more details.
