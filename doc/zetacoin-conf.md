...
The configuration file is used by `zetacoind`, `zetacoin-qt` and `zetacoin-cli`.

All command-line options (except for `-?`, `-help`, `-version` and `-conf`) may be specified in a configuration file, and all configuration file options (except for `includeconf`) may also be specified on the command line. Command-line options override values set in the configuration file and configuration file options override values set in the GUI.

Changes to the configuration file while `zetacoind` or `zetacoin-qt` is running only take effect after restarting.
...
The `settings.json` file contains dynamic settings that are set by the Zetacoin Core GUI and RPCs at runtime, and augment or replace the static settings specified in the `zetacoin.conf` file.

Command line options also augment or replace `zetacoin.conf` options, and can be useful for scripting and debugging.
...
The configuration file is not automatically created; you can create it using your favorite text editor. By default, the configuration file name is `zetacoin.conf` and it is located in the Zetacoin data directory, but both the Zetacoin data directory and the configuration file path may be changed using the `-datadir` and `-conf` command-line options.

The `includeconf=<file>` option in the `zetacoin.conf` file can be used to include additional configuration files.

### Default configuration file locations

Operating System | Data Directory | Example Path
-- | -- | --
Windows | `%LOCALAPPDATA%\Zetacoin\` | `C:\Users\username\AppData\Local\Zetacoin\zetacoin.conf`
Linux | `$HOME/.zetacoin/` | `/home/username/.zetacoin/zetacoin.conf`
macOS | `$HOME/Library/Application Support/Zetacoin/` | `/Users/username/Library/Application Support/Zetacoin/zetacoin.conf`
...
The output is placed under `share/examples/zetacoin.conf`.
To use the generated configuration file, copy the example file into your data directory and edit it there, like so:

```
# example copy command for linux user
cp share/examples/zetacoin.conf ~/.zetacoin
```
