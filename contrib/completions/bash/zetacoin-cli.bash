# bash programmable completion for zetacoin-cli(1)
# Copyright (c) 2012-2022 The Zetacoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

# call $zetacoin-cli for RPC
_zetacoin_rpc() {
...
_zetacoin_cli() {
    ...
    # save and use original argument to invoke zetacoin-cli for -help, help and RPC
    # as zetacoin-cli might not be in $PATH
    zetacoin_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($zetacoin_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_zetacoin_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _zetacoin_cli zetacoin-cli
