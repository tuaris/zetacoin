...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " zetacoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The zetacoin-tx tool is used for creating and modifying zetacoin transactions.\n\n"
                "zetacoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded zetacoin transaction, or with \"-create [commands]\" to create a hex-encoded zetacoin transaction.\n"
                "\n"
                "Usage: zetacoin-tx [options] <hex-tx> [commands]\n"
                "or:    zetacoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded zetacoin transaction
