...
Options:
  -m, --multiprocess     Run multiprocess binaries zetacoin-node, zetacoin-gui.
  -M, --monolithic       Run monolithic binaries zetacoind, zetacoin-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'zetacoin-qt [ARGS]' or 'zetacoin-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'zetacoind [ARGS]' or 'zetacoin-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'zetacoin-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'zetacoin-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'zetacoin-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_zetacoin [ARGS]'.
  chainstate [ARGS] Run zetacoin kernel chainstate util, equivalent to running 'zetacoin-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_zetacoin [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_zetacoin-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "zetacoin-gui" : "zetacoin-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "zetacoin-node" : "zetacoind");
        } else if (cmd.command == "rpc") {
            args.emplace_back("zetacoin-cli");
            ...
            args.emplace_back("zetacoin-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("zetacoin-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_zetacoin");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("zetacoin-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_zetacoin");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_zetacoin-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("zetacoin-util");
        } else {
