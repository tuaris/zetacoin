...
const char * const BITCOIN_CONF_FILENAME = "zetacoin.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Zetacoin
    //   new: C:\Users\Username\AppData\Local\Zetacoin
    // macOS: ~/Library/Application Support/Zetacoin
    // Unix-like: ~/.zetacoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Zetacoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Zetacoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Zetacoin";
#else
    // Unix-like
